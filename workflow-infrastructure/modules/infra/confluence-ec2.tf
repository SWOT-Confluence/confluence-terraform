...
resource "aws_launch_template" "aws_ec2_lt_efs" {
  ...
  user_data              = base64encode(templatefile("${path.module}/scripts/ec2-efs-user-data.sh", { logs = aws_efs_file_system.efs_fs_logs.id, validation = aws_efs_file_system.efs_fs_val.id, offline = aws_efs_file_system.efs_fs_off.id, output = aws_efs_file_system.efs_fs_out.id, diagnostics = aws_efs_file_system.efs_fs_diag.id, moi = aws_efs_file_system.efs_fs_moi.id, input = aws_efs_file_system.efs_fs_in.id, flpe = aws_efs_file_system.efs_fs_flpe.id, coastalq = aws_efs_file_system.efs_fs_coastalq.id }))
}
